...
public class HCSR04Device implements ISensor {
     ...
     *            the echo
     */
    public HCSR04Device(int trigger, int echo) {
    ...
    public double getDistance() {
        ...
        while ((!echo.isHigh()) && (start < starttime + DELAY * TWO)) {
            // System.out.println("dentro do ṕrimeiro while. echo.state:" +
            // echo.getState() + " trigger.state: " + trigger.getState());
            start = System.nanoTime();
        }
        System.out.println("saiu do primeiro while. [echo.state:"
                + echo.getState() + "] [trigger.state: " + trigger.getState()
                + "]");
        while ((echo.isHigh()) && (stop < starttime + DELAY * TWO)) {
            // System.out.println("dentro do segundo while. echo.state:" +
            // echo.getState() + " trigger.state: " + trigger.getState());
            stop = System.nanoTime();
        }
        long delta = (stop - start);
        System.out.println("saiu do segundo while. [start: " + start
                + "] [stop: " + stop + "] [delta: " + delta);
        // echo from 0 to 1 depending on object distance cm/s
        distance = delta * SPEEDOFSOUND;
        System.out.println("distance: " + distance);
        System.out
                .println("distance calculated: " + (distance / 2.0 / (DELAY)));
